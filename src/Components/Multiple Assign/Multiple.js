...
function Multiple() {
    ...
    if (showMentor == '') {
      ...
      try {
        let filterData = await axios.get(
          `${env.api}/filter-student/${showMentor}`
        );
        if (filterData.data == '') {
          setStudData([]);
  ...
  const assignMultiple = async () => {
    if (assignMentor == '') {
      ...
      try {
        await axios.put(`${env.api}/update-multiple`, {
          assignMentor,
          showMentor,
        });
        setStudData([]);
      ...
      <div className='row'>
        <div className='col col-lg-6'>
          ...
          <div className='input-group mb-3'>
            <select
              ...
              value={showMentor}
              onChange={(e) => setShowMentor(e.target.value)}
              required
            >
              <option defaultValue></option>
              {mentorName.map((obj) => {
                return (
                  <option value={obj.mentor_details.Name}>
                    {obj.mentor_details.Name}
                  </option>
                );
              })}
            </select>
            ...
            <button className='btn btn-danger' onClick={clearshowmentor}>
        ...
        <div className='col col-lg-6'>
          ...
          <div className='input-group mb-3'>
            ...
            <button className='btn btn-success' onClick={assignMultiple}>
              <i className='fas fa-plus'></i>
            </button>
            <button className='btn btn-danger' onClick={clearassignmentor}>
              <i className='far fa-times-circle'></i>
            </button>
          </div>
        </div>
